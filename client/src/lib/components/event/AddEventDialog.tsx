...
const addEventBtnSx = { fontWeight: 700, mt: 5 }

export default function AddEventDialog({ open, setOpen }: AddEventDialogProps) {
  return (
    <Dialog open={open} setOpen={setOpen}>
      <DialogTitle sx={dialogTitleSx}>Add Event</DialogTitle>
      <form className="flex flex-col m-10">
        <TextField label="Event Name" required sx={mb2Sx} />
        <TextField label="Event Description" multiline rows={3} sx={mb2Sx} />
        <TextField label="Location" required sx={mb2Sx} />
        ...
        <Button variant="contained" sx={addEventBtnSx}>
          Add Event
        </Button>
      </form>
    </Dialog>
